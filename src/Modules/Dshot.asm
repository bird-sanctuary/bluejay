...
DSHOT_RCPULSE_STATE_BOOST           EQU 8       ; stall boost state
DSHOT_RCPULSE_STATE_PWM_LIMIT       EQU 10      ; apply pwm limit state
DSHOT_RCPULSE_STATE_DYNAMIC_PWM     EQU 12      ; choose dynamic pwm frequency state
DSHOT_RCPULSE_STATE_LSD             EQU 14      ; limit pwm, scale & calculate dithering pattern
DSHOT_RCPULSE_STATE_SET_DAMP        EQU 16      ; set damp state
DSHOT_RCPULSE_STATE_SET_PWM         EQU 18      ; set pwm state
...
dshot_rcpulse_stm_start_state:
    ...
    ; setb    Flag_Rcp_Stop ; It is set below when it finds that Temp5:4 is 0
    mov Temp4, #0
    mov Temp5, #0
...
dshot_rcpulse_stm_filter_state:
    ...
    ; rcpulse_new = (rcpulse + rcpulse_prev) / 2

    ; Skip filter if it is explicitly ordered by the user
    mov Temp1, #Pgm_Enable_RcPulse_Filter
    mov A, @Temp1
    ...
    sjmp dshot_rcpulse_stm_filter_state_done
