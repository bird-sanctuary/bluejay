...
Flag_32o16ms_Elapsed			BIT	Flags2.3		; Set when timer2 interrupt is triggered
...
ENDIF
	...
	movx @Temp1, A					; Clear XRAM address
...
arming_begin:
	...
	mov	Stall_Counter, #0		; Reset stall count
...
motor_start_bidir_done:
    ; Prepare states and counters
	setb	Flag_Startup_Phase		  ; Set startup phase flags
	setb	Flag_Initial_Run_Phase
	mov	Startup_Cnt, #0			      ; Reset startup phase run up counter
	mov	Initial_Run_Rot_Cntd, #12	  ; Set initial run rotation down counter
    ...
    mov Wt_Zc_Scan_Time_Quanta_L, #000h
    mov Wt_Zc_Scan_Time_Quanta_H, #004h

    ; Kick motor to startup
	call   comm5_comm6
...
exit_run_mode_on_timeout:
	inc	Stall_Counter			; Increment stall counter
...
ENDIF
	...
	; Otherwise - it's a stall
	; Stalls are a dangerous situation, they can cause motors and escs to burn
	; Maximum consecutive stalls have to be limited to avoid damage
	clr	C
	mov	A, Stall_Counter
	subb	A, #3					; 3 Maximum consecutive stalls
